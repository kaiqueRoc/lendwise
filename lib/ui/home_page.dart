import 'dart:convert';
import 'package:lendwise/ui/simulate_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void getInstitutions() async {
    var response = await Dio().get('http://localhost:45327/api/instituicao');
  ...
  void getCovenants() async {
    try {
      var response = await Dio().get('http://localhost:45327/api/convenio');

      if (response.statusCode == 200) {
        final String jsonString = response.data;

        // Verifique se a resposta parece ser JSON antes de tentar analisá-la.
        if (jsonString.startsWith('{"') && jsonString.endsWith('"}')) {
          final dynamic jsonDecoded = json.decode(jsonString);

          setState(() {
            covenantsList = jsonDecoded;
          });
        } else {
          // A resposta não parece ser JSON válido.
          print('A resposta não é um JSON válido: $jsonString');
        }
      } else {
        // Trate o código de status da resposta que não seja 200 de acordo com suas necessidades.
        // Por exemplo, você pode mostrar uma mensagem de erro ao usuário.
        print('Erro na solicitação: ${response.statusCode}');
      }
    } catch (e) {
      // Lidar com exceções que podem ocorrer durante a solicitação ou análise JSON.
      print('Erro durante a solicitação ou análise JSON: $e');
    }
  }
